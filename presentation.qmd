...
3. **Example 2**: Age + Gender → Outcome (Simpson's paradox)
4. **Example 3**: Age + Gender + Gender Opportunity → Outcome (limits of manual standardization)
...
# Example 2: Age + Gender {.section-break background-color="#2C3E50"}
...
sim2_data %>%
  ...
  labs(
    title = "Age Distribution by Gender (Dataset 2)",
...
::: {.fragment}
**Key:** P(Age | Man) ≠ P(Age | Woman), but **marginal (overall) P(Age) same as Dataset 1**
:::
...
ggplot(crude_sex, aes(x = sex, y = crude_rate, fill = sex)) +
  ...
  scale_fill_manual(values = c("#3498DB", "#E74C3C")) +
  labs(
    title = "Crude Late-Presentation Rates by Gender",
...
::: {.incremental}
- $P(\text{Age} \mid \text{Man})$ ≠ $P(\text{Age} \mid \text{Woman})$
...
**Estimate what each group's outcomes *should look like***
if they experienced the **same rates as a reference population**
...
::: {.fragment}
**Interpretation:** Men have `r sprintf("%.0f%%", (indirect_std$SMR[indirect_std$sex == "Men"] - 1) * 100)` higher risk than expected from standard rates
:::
...
::: {.incremental}
- **Crude rates get it wrong** because Gender → Age and Age → Outcome
...
# Example 3: Age + Gender + GOC {.section-break background-color="#2C3E50"}
...
- Gender effect: Men higher within age groups (constant logit-scale effect)
...
sim3_data %>%
  ...
  scale_fill_manual(values = c("#3498DB", "#E67E22", "#E74C3C")) +
  labs(
    title = "Age Distribution by Gender and GOC (Dataset 3)",
    subtitle = "Complex patterns: P(Age | Gender, GOC) varies, but marginal (i.e., overall) P(Age) same as D1/D2",
    x = "Age Group",
    y = "Proportion",
    fill = "Age"
  ) +
...
::: {.fragment}
**All confounded!**
Low GOC appears to have every so slightly lower risk (wrong!) because low GOC contexts are younger.
:::
...
::: {.incremental}
- Need age-GOC-specific rates for each gender: 3 ages × 3 GOC = **9 strata per gender**
...
$$
\begin{aligned}
\log(E[Y \mid \text{Gender}, \text{Age}, \text{GOC}]) = \\
\beta_0 + \beta_{\text{Gender}} + \beta_{\text{Age}} + \beta_{\text{GOC}}
\end{aligned}
$$
...
summary_comparison <- tribble(
  ...
  "Crude",
  "Women appear worse (incorrect)",
  "Low GOC appears better (incorrect)",
