...
ggplot(true_rates_sim1, aes(x = age_group, y = true_rate, fill = age_group)) +
  ...
  scale_fill_manual(values = c("#3498DB", "#E67E22", "#E74C3C")) +
  labs(
    title = "Late Presentation Age-Specific Rates",
    subtitle = "We set these from simulated data, so we know the truth/unbiased rates",
    x = "Age Group",
...
standard_rates <- data.frame(
  age_group = factor(c("Young", "Middle", "Older"),
                    levels = c("Young", "Middle", "Older")),
  std_rate = c(0.04, 0.14, 0.42)  # Externally-determined rates; they differ a little
)

# Get population counts by sex and age
...
```{r}
#| label: indirect-std-sim2-calc
#| echo: true

# For each sex, calculate observed and expected events
indirect_std <- pop_counts |>
  left_join(standard_rates, by = "age_group") |>  # Merge in our rates
  left_join(age_sex_rates, by = c("sex", "age_group")) |>
  group_by(sex) |>
  summarise(
...
```{r}
#| label: direct-std-sim2-calc
#| echo: true

# Apply standard weights to sex-age specific rates
