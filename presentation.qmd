...
# Render DAG images (pre-render to avoid knitr bugs)
source("scripts/render_dags.R")

# Load simulation datasets from Framework data catalog
base_population <- data_load("base_population")
sim1_data <- data_load("sim1_data")
sim2_data <- data_load("sim2_data")
sim3_data <- data_load("sim3_data")

# ENFORCE FACTOR ORDERING FOR ALL DATASETS
# Age group: Young -> Middle -> Older
sim1_data <- sim1_data |>
  mutate(age_group = factor(age_group, levels = c("Young", "Middle", "Older")))
...
```{r}
#| label: crude-gender-sim2
#| echo: false

crude_gender <- sim2_data |>
  group_by(gender) |>
  summarise(crude_rate = mean(late_presentation))

ggplot(crude_gender, aes(x = gender, y = crude_rate, fill = gender)) +
  geom_col(width = 0.6) +
...
```{r}
#| label: age-specific-gender-sim2
#| echo: true
...
age_gender_rates_plot <- age_gender_rates |>
  mutate(age_group = factor(age_group, levels = c("Young", "Middle", "Older"), ordered = TRUE))

ggplot(age_gender_rates_plot, aes(x = age_group, y = rate,
...
ggplot(indirect_std, aes(x = gender, y = SMR, fill = gender)) +
  geom_col(width = 0.6) +
...
::: fragment
**Interpretation:** Men have `r sprintf("%.0f%%", (indirect_std$SMR[indirect_std$gender == "Men"] - 1) * 100)` higher risk than expected from standard rates
:::
...
comparison_sim2 <- bind_rows(
  crude_gender |> select(gender, rate = crude_rate) |> mutate(type = "Crude"),
  direct_std |> select(gender, rate = standardized_rate) |> mutate(type = "Standardized")
)

ggplot(comparison_sim2, aes(x = gender, y = rate, fill = type)) +
...
```{r}
#| label: age-by-gender-goc-sim3
#| echo: false
#| fig-width: 12

sim3_data |>
  count(gender, goc, age_group) |>
  group_by(gender, goc) |>
  mutate(prop = n / sum(n)) |>
  ggplot(aes(x = age_group, y = prop, fill = age_group)) +
  geom_col() +
...
::: fragment
**Men have 2.8 percentage points higher risk than women** (after adjusting for age and GOC)
:::
...
ewestlund\@jhu.edu<br> Johns Hopkins Biostatistics Center<br> Johns Hopkins Bloomberg School of Public Health<br>

<br>

Slides and code: [github.com/erikwestlund/gdaw-standardization-presentation-2025](https://github.com/erikwestlund/gdaw-standardization-presentation-2025)